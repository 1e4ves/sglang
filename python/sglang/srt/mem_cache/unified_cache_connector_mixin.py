...
class UnifiedCacheConnectorMixin:
    ...
    def init_connector(self, server_args: ServerArgs, params: CacheInitParams) -> None:
        ...
        self.tree_core.enable_connector = True
    ...
    def _connector_tail_keys(
        ...
        if device_hit_len > 0:
            ...
            last_hash = (
                self.tree_core.get_last_hash_value(node_id)
    ...
    def load_connector(self, req: Req) -> tuple[torch.Tensor, NodeId]:
        empty = self.tree_core.empty_match_result.device_indices
        ...
        node = self.tree_core.node_by_id(loaded.last_device_node)
    ...
    def _retarget_connector_load(
        ...
        for transfer in transfers:
            if transfer.name == PoolName.KV:
                ...
                canonical_mamba = (
                    self._resolve_connector_node(loaded.last_device_node)
    ...
    def _resolve_connector_node(
        self, node: NodeId | UnifiedTreeNode
    ) -> UnifiedTreeNode:
        if isinstance(node, int):
            return self.tree_core.node_by_id(node)
        return node

    def offload_connector_node(self, node: NodeId | UnifiedTreeNode) -> None:
        node_obj = self._resolve_connector_node(node)
        transfers = []
        for component in self._components_tuple:
            transfer = component.build_connector_transfer(
                ConnectorTransferPhase.OFFLOAD, node=node_obj
            )
        ...
        node_obj.connector_offloaded = True
    ...
    def drain_connector_offloads(self) -> None:
        ...
        for _ in range(count):
            ...
            node_obj = self._resolve_connector_node(node)
            node_obj.connector_offloaded = self.connector.pop_completed_offload()
            self.dec_lock_ref(node, lock_params)
