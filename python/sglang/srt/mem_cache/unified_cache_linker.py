...
class UnifiedCacheLinkerWrapper:
    ...
    def load_back(self, req: Req) -> tuple[torch.Tensor, NodeId]:
        ...
        local_success = self.cache_linker.load(req.rid, load_transfers)
        if not self._sync_success(local_success):
            if local_success:
                self.cache_linker.cancel_queued_load(req.rid)
    ...
    def _sync_success(self, success: bool) -> bool:
        """Make every attention rank take the same branch after queuing I/O."""
        synced = torch.tensor([int(success)], dtype=torch.int)
        self.cache._all_reduce_attn_groups(synced, torch.distributed.ReduceOp.MIN)
    ...
    def release_request(self, rid: str) -> None:
        ...
        self.cache_linker.cancel_queued_load(rid)
